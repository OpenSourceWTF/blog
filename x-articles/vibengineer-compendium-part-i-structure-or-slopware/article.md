...
[INSERT IMAGE: vibengineer-01-structure-or-slopware.jpg]

ALT: A coding prompt splits into slopware on one side and a maintainable result guided by intent, process, state, and evidence on the other. The Vibengineer redirects an eager Agent toward the structured path.

CAPTION: Intent, process, state, and evidence are what separate forward progress from slopware.
...
[INSERT IMAGE: vibengineer-02-guidance-vs-enforcement.jpg]

ALT: The Agent follows a Post-it saying “Tell me how to do it,” while the Vibengineer points to increasingly strong controls from instructions through tests and CI.

CAPTION: Instructions guide. Permissions, tests, and CI can enforce.
...
0. **No full framework** — for small, reversible, well-understood changes. You supply the judgment and remember the real checks.
1. **OpenSpec** — for lightweight, resumable change specs in an existing repository. You still supply human review and external test, CI, and security gates.
2. **Superpowers** — for portable design, planning, TDD, review, and completion discipline. You accept more checkpoints than small work needs.
3. **GitHub Spec Kit** — for governed, portable spec-to-implementation artifacts and workflow gates. You maintain more artifacts, templates, integrations, and governance.
4. **BMAD** — when product, architecture, development, and QA need explicit artifact handoffs. You steward many roles, documents, and state transitions.
5. **Superpowers Optimized** — for proportional routing, deep verification, persistent project state, and Codex hooks. You accept the largest local policy surface here, fork maintenance, and lower portability.
...
[INSERT IMAGE: vibengineer-03-one-readable-truth.jpg]

ALT: The Vibengineer and Agent read the same linked project map, decisions, state, and known-issues documents. Search, RAG, graph, and DAG views are derived from that one readable source.

CAPTION: Keep one readable source of truth. Treat retrieval systems as derived views.
