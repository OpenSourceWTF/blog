...
They were not broken kernels. Several were two or three
...
It turns out that the question I had been asking was wrong. I was asking how to
make the GPU finish its work faster. I had not established whether the GPU was
the part holding up the cycle. For models with so few active parameters, CPU
operations affected speed far more than I expected.
...
intermediate tensors, cache bookkeeping, and CPU/GPU round trips. First, the
byte-identical path reached 188.7 tokens per second. A separate, non-byte-identical
...
profiler I did not expect to have to build. I hope that, if you are in the
optimization game, this article serves as an interesting compendium of tools
and techniques for MLX LLM kernel optimization.
...
## 4. Second Holistic CPU/GPU Pass
